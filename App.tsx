...
import { Home, Plus, Briefcase, DollarSign, FileText, Calendar, Map, User, Bot, LogOut, ChevronLeft } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    ...
    if (currentView === 'profile') return <ProfileView onUpdate={handleProfileUpdate} />;
    
    if (!selectedTripId) return <HomeView onSelectTrip={navigateToTrip} />;

    switch (currentView) {
      case 'trip-dashboard': return <TripDashboard tripId={selectedTripId} onNavigate={navigateTo} />;
      ...
      case 'documents': return <div className="bg-slate-50 min-h-full"><DocumentsView tripId={selectedTripId} /></div>;
